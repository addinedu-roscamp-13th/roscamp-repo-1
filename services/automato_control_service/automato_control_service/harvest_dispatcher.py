...
■ 채울 때 확정할 설계 노트
  (1) 주행 재사용 — E2/E5 주행은 순찰의 '예약하며 Navigate' 루프를 재사용해야 한다.
      PatrolDispatcher 안에 '목적지 노드까지 예약·주행'하는 조각이 있으면 추출해 공유한다.
...
class HarvestDispatcher:
    ...
    def __init__(self, logger):
        self._log = logger
        # 노드가 라우팅 그래프를 로드할 때 채워준다(Navigate 골 구성용 좌표).
        # 순찰 디스패처(PatrolDispatcher.wp_meta)와 같은 규약을 쓴다.
        self.wp_meta = {}
