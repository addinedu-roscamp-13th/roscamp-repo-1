...
- **`charuco_boards` 신설 후 폐기(0010)**: v33에서 정밀 도킹용 ChArUco 보드/도킹 오프셋 테이블로 신설했으나, 도킹 방식이 지점별 라우팅(충전소=반사테이프, 수확·예냉실=H마커 바닥 도킹)으로 바뀌어 ChArUco 도킹을 더 이상 쓰지 않게 되면서 0010 마이그레이션에서 DROP 했다.
