...
# 성공 실측 대역 — 수확 성공 15건(2026-07-28 6건 + 07-29 9건)의 포락선.
#   x 239.6~271.9   y -9.5~143.2   z 231.6~357.2
# x·z 만 제약으로 본다. 실패 원인이 된 것이 이 둘뿐이기 때문이다 — z 가 낮으면
# standoff(진입 45mm 뒤 지점)를 못 잡고, x 가 크면 팔이 뻗어 정확도가 떨어진다.
# y 로 인한 실패는 관측된 적이 없어 베드 폭을 덮도록 여유 있게 둔다.
#
# ⚠ 이 안에 있다고 반드시 따는 것은 아니다(필요조건이지 충분조건이 아니다). 대역
# 안에서도 '빈손' 실패가 나오는데, 그건 검출 좌표의 ~9mm 오차가 20mm 열매를
# 만나서 생기는 것이라 위치로는 못 없앤다. 대역 **밖**은 거의 확실히 실패한다.
SUCCESS_ZONE_MM = {"x": (240.0, 272.0), "y": (-20.0, 150.0), "z": (232.0, 357.0)}
