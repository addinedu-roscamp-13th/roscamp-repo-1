...
"""
import argparse, json, math, threading, time, os
import paramiko
import rclpy
...
class Bridge(Node):
    def __init__(self, euler):
        ...
        # 로봇 IP: 학원망에선 192.168.0.2 (DHCP). 바뀌면 ROBOT_IP 환경변수로 지정.
        robot_ip=os.environ.get("ROBOT_IP","192.168.0.2")
        self.ssh.connect(robot_ip,username="jetcobot",password="1",timeout=10)
