...
으로 그대로 불러오되, GroupAction + PushRosNamespace 로 `/<namespace>` 접두어를 씌운다.
→ 드라이버 토픽이 `/dg_01/ddago/odom`, `/dg_01/ddago/batt_state` 처럼 로봇별로 갈라진다.
...
같은 네임스페이스를 주어 별도 기동한다:
  ros2 launch pinky_navigation localization_launch.xml namespace:=dg_01/ddago ...
  ros2 launch pinky_navigation navigation_launch.xml   namespace:=dg_01/ddago ...

이 wrapper 는 핑키 드라이버(pinky_bringup)가 설치된
실제 로봇(또는 로봇 PC)에서 실행한다. 드라이버가 없는 개발 PC 에서는 뜨지 않는다.

실행 예:
  ros2 launch ddago_control ddago_bringup.launch.py                          # 기본 dg_01/ddago
  ros2 launch ddago_control ddago_bringup.launch.py namespace:=dg_02/ddago   # 다른 로봇
"""
...
def generate_launch_description():
    ...
    return LaunchDescription([
        DeclareLaunchArgument(
            ...
            description='드라이버 토픽/TF 앞에 붙는 네임스페이스 (view·nav2 와 동일하게 맞춘다)',
        ),
        GroupAction([
            # 이 그룹 안의 모든 노드/포함 launch 에 /<namespace> 네임스페이스 주입
