...
class PatrolServer(Node):
    def __init__(self, **kwargs):
        ...
        # 그래서 goal 을 받을 때마다 task_id 를 토픽으로 알려준다.
        #   latched(TRANSIENT_LOCAL, depth 1): 구독자가 나중에 떠도 마지막 값을 즉시
        #   받는다. 덕분에 이 값을 주기적으로 재발행할 필요가 없다.
        #   ddago·ddagi 가 같은 망을 공유하므로 telemetry 와 같은 규칙으로 /ddago 접두어.
    ...
    def _execute(self, goal_handle):
        ...
        self._publish_current_task(req.task_id)
