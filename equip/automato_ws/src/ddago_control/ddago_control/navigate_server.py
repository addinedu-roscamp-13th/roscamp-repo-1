...
class NavigateServer(Node):
    def __init__(self, **kwargs):
        ...
        self._latest_frame = None   # 카메라가 상시 갱신하는 최신 Image (촬영 시 grab)
        ...
        self.get_logger().info(
            ...
            f'카메라={camera_topic}, 분석={analyze_service}')
    ...
    # ------------------------------------------------------------------ #
    # 촬영: settle 대기 → 프레임 grab → analyze_frame 호출(fire-and-forget)
    # ------------------------------------------------------------------ #
    def _capture_and_request(self, task_id, waypoint_id):
        # 정지 잔상 방지용 짧은 대기 후 최신 프레임 스냅샷.
        # grab 자체는 멤버변수 복사라 즉시 끝나고, 분석은 응답을 안 기다리므로
        # 다음 waypoint 주행이 settle 시간(기본 0.3초)만큼만 늦어진다.
        time.sleep(self._settle_sec)
        frame = self._latest_frame
        if frame is None:
            self.get_logger().warn(
                f'waypoint={waypoint_id} 촬영 대상이나 카메라 프레임 미수신 → '
                f'분석요청 스킵(주행은 계속). 카메라 토픽/드라이버 확인 필요')
            return
